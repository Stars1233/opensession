...
test.each([false, true])(
  "teardown posts one keepalive report with counters, never transcript or audio (native: %s)",
  async (native) => {
    install("MediaStream", {
      value: class {
        getTracks() {
    ...
    const nativeStart = native
      ? spyOn(NativeVoiceAudio.prototype, "start").mockResolvedValue(
          new MediaStream(),
        )
      : null;
    const nativeStop = native
      ? spyOn(NativeVoiceAudio.prototype, "stop").mockImplementation(() => {})
      : null;
    const nativeOutput = native
      ? spyOn(NativeVoiceAudio.prototype, "attachOutput").mockImplementation(
          () => {},
        )
      : null;
    ...
    install("window", {
      value: Object.assign(windowEvents, {
        setTimeout,
        clearTimeout,
        os1: native ? { voiceAudio: {} } : undefined,
      }),
    });
    ...
    install("RTCPeerConnection", {
      value: class {
        ...
        async setRemoteDescription() {
          if (native) this.ontrack?.({ streams: [new MediaStream()] });
    ...
    try {
      ...
      expect(browserCaptures).toBe(native ? 0 : 1);
      if (native) {
        expect(nativeStart).toHaveBeenCalledTimes(1);
        expect(nativeOutput).toHaveBeenCalledTimes(1);
