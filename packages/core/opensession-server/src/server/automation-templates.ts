...
3. Investigate the actual issue: reproduce the failure path in the code if relevant, check Sentry for matching errors, and search the repo's GitHub issues for known ones (\`gh issue list --repo ${defaultRepo().ghRepo} --search\`). Link the thread to the best matching open issue with the Plain \`link_thread_to_github_issue\` tool. When that issue is a feature request and the customer explained their use case, add it as a comment (\`gh issue comment <n> --repo ${defaultRepo().ghRepo} --body\`): what they are trying to do and why, in your own words, plus a \`- Plain thread: <thread URL>\` line. Leave out the customer's name and email, and skip the comment when they only asked for the feature without saying what they need it for. Create a new issue (\`gh issue create --label feature-request\`) only for a feature request no open issue tracks, then link the thread to it. Never close or reopen an issue that still has Plain threads linked; that flips those threads to Close the loop.
...
5. If you found a real bug with a clear fix, implement it in your worktree and open a PR for review — mention the PR in the note. Then link the thread to the PR with the same \`link_thread_to_github_issue\` tool, passing the PR number: Plain shows it under the thread's links and moves the thread to Close the loop when the PR merges or closes.
